...
def train_BCE_WAE(args, model, domain):
    train_wavelet_autoencoder(args, model, domain, BCE_Loss=True)

def train_MSE_WAE(args, model, domain):
    train_wavelet_autoencoder(args, model, domain, BCE_Loss=False)
...
def get_wae_config(args, model, domain, BCE_Loss):
    print("Preparing training D1 for %s"%(domain.name))
    ...
    config.name = 'wae_%s_%s'%(domain.name, model.preferred_name())
...
def train_wavelet_autoencoder(args, model, domain, BCE_Loss):
    if BCE_Loss:
        model.netid = "BCE." + model.netid
    else:
        model.netid = "MSE." + model.netid
    ...
    if not os.path.isfile(hbest_path+".done"):
        config = get_wae_config(args, model, domain, BCE_Loss=BCE_Loss)
        trainer = IterativeTrainer(config, args)
        print('Training from scratch')
        best_loss = 999999999
        for epoch in range(1, config.max_epoch+1):
